...
모델 작업은 대기열 칩("요약 중 · 대기 N · 약 Ns")으로 보이고, 행에 마우스를 올리면 `완료`/`나중에` 빠른 동작. 마무리 시각(설정, 기본 17:30) 이후 오늘 마무리 알림, 월요일 아침 주간 리포트 알림 — 벨 뱃지의 알림함에 쌓임.
`-Panel -Watch` 는 Outlook 에서 **지금 선택한 메일**도 5초마다 내보내서(`web\data\selected.json`) 컴포저에 핀 태그로 뜨고, 팔레트 첫 후보가 "이 메일 요약 / 이 메일 답장 초안"이 된다. 컴포저에 `설정` 이라고 치면 모델·마무리 시각·알림·캐시 설정.
